...
trajLength = 20000 # (100, 500, 2000, 20000) arbitrary large number 
...
set.seed(47405) 
# Specify standard deviation of proposal distribution (step size sigma): 
proposalSD = c(0.02,0.2,2.0)[3] 

# for this for-loop, input is the posteriordensityfunction "targetRelProb"
# output is "trajectory" over many iterations
for ( t in 1:(trajLength-1) )  {
  currentPosition = trajectory[t] # theta[t]
  # Use the proposal distribution to generate a proposed jump. 
  proposedJump = rnorm( 1 , mean=0 , sd=proposalSD ) # symmetric random walk
  ...
  if ( runif(1) < probAccept )  {
    ...
    trajectory[ t+1 ] = currentPosition + proposedJump # theta(t+1)
...
hist(trajectory[idxToPlot], freq = FALSE, xlab = '', main = 'Metropolis Sampling', xlim = c(0, 1)) 
# iid sample based on analytical solution (Beta-Binomial model)
hist(rbeta(1000, sum(myData) + 1, sum(1-myData) + 1), xlab = '', freq = FALSE, main = 'Analytical Solution', xlim = c(0, 1))
